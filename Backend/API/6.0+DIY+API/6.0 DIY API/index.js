...
app.get("/random",async (req,res)=>{
  try{
    ...
    res.status(201).json(jokes[rand]);
  }catch(error){
...
app.get("/jokes/:id",async (req,res)=>{
  try{
    ...
    const jokef = jokes.find((joke) => joke.id == id);
    res.status(201).json(jokef);
  }catch(error){
    res.sendStatus(404).send(error.response.data)
  }
})
...
app.get("/filter",async(req,res)=>{
  try{
    const type = req.query.type; //string
...
app.put("/jokes/:id", async (req,res)=>{
  try{
    ...
    const repId = jokes.findIndex((joke)=> joke.id == jid);
    jokes[repId] = replacement;
...
app.delete("/jokes/:id",async (req,res)=>{
  try{
    ...
    const delIndex = jokes.findIndex((joke)=> joke.id == id);
    
    if(id>= 0){
      jokes.splice(delIndex,1);
      res.sendStatus(201);
    }
    else{
